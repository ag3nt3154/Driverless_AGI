...
**Slash commands:** same set as TUI — `/help`, `/clear`, `/model`, `/compact`, `/tools`, `/skills`, `/workflows`, `/hist`, `/init`, `/copy`, `/exit`, `/show-pet`. `SlashCommandHandler.completions()` exposes all valid command names + descriptions for autocomplete (slash-command autocomplete popup in progress).
