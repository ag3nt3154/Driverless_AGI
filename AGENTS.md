...
- **TUI**: horizontal layout — `#main-column` (1fr) left, `Sidebar` (30%, max 45) right. `/copy` copies `ConversationPane._last_assistant` to OS clipboard (clip/pbcopy/xclip). `StreamPreview` expands on first delta, collapses on `on_stream_end`. `_model_name` derived from resolved config.
