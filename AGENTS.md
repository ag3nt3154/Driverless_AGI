...
| `tools/_handoff_format.py` | `format_handoff_result(handoff_path, unverified=False)` — shared rendering for `spawn_subagent`/`cli_subagent`/`extend_timeout` results: inlines handoff content, prepends a warning banner when `unverified=True`. `dispatch_status_result(result, error_prefix, include_escalation=False, include_timeout=True)` — shared `run_subagent`/`resume_subagent` result-dict → tool-return-string dispatch, used by all 5 subagent-spawning tools. `unverified_flag_path(handoff_path)` — shared `<stem>_unverified.flag` sidecar path builder, used by both `tools/subagent_main.py` (writer) and `tools/_subagent_runner.py` (reader) |
...
- **Post-merge ponytail review cleanup (2026-07-26)**: 5 findings fixed directly on top of the subagent-handoff-and-briefing merge — see the TODO.md Completed entry for full detail. In short: `_dispatch_result`/dispatch-branch duplication across 5 spawn tools centralized into `dispatch_status_result()`; `SpawnSubagentTool.__init__`'s double YAML parse fixed; `agent/loop.py`'s `_handle_write_handoff` (which had a dropped list-safety conversion and a 6-positional-param violation) refactored to share `_bookkeep_tool_call()`/`_finalize_turn()` with the normal dispatch loop; the `<stem>_unverified.flag` path-construction duplication between `subagent_main.py`/`_subagent_runner.py` centralized into `unverified_flag_path()`; `write_handoff/_write_handoff.py`'s stale "planned change" docstring corrected. `agent/loop.py`'s pre-existing size (1172 lines, over the 500-line cap) and `AgentLoop.run`'s CC 48 (over the CC<=8 cap) were noted as separate, larger pre-existing violations and spun off as a standalone follow-up task rather than folded into this cleanup.
