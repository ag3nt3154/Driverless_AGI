...
- **2026-08-07**: DeepSeek thinking-mode API rejected multi-turn requests — `reasoning_content` was never captured from streaming deltas (`model_extra` fallback only checked key `"reasoning"`, not `"reasoning_content"`) and never echoed back → fixed `_consume_stream` fallback + `_extract_reasoning` to check both keys; both assistant-message append sites now echo `reasoning_content` via `_extract_reasoning(message)`.
