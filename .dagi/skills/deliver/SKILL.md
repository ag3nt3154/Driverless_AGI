...
4. Read the review handoff.
   ...
   - **ESCALATE**: enter plan mode, address blocking findings, exit, then repeat from
     step 3. Ask the user only if a finding requires a decision outside the original scope.
...
2. If `READY_FOR_REVIEW`:
   ...
   d. **ESCALATE**: Diagnose the findings. Either: assign a targeted repair to a new
      worker call, or enter plan mode to revise the subtask, then repeat from step 1.
      Worker debugging continues locally — no fixed attempt count.
