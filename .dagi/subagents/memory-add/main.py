...
class SpawnMemoryAddSubagentTool(BaseTool):
    ...
    description = (
        "File a new entry into the memory wiki. Use to persist a decision, "
        "design note, known issue, or any information worth remembering across "
        "sessions."
    )
    _parameters = {
        "type": "object",
        "properties": {
            ...
            "custom_instructions": {
                "type": "string",
                "description": (
                    "Additional guidance: topic placement, related pages, "
    ...
    def run(self, task: str, custom_instructions: str = "") -> str:
        ...
        if self._callbacks and self._callbacks.on_subagent_event_factory:
            on_event = self._callbacks.on_subagent_event_factory("memory-add")
