...
> Session 2026-06-13i: Added `web_search` and `web_fetch` tools to plan-mode registry unconditionally. Previously they only appeared in plan mode when `config is None` (test/fallback path); production runs with a real config only had the subagent launchers. Fix in `agent/tools.py`: moved tool registration before the `if config is not None:` subagent block and removed the now-redundant `else` branch. 116/117 tests pass (1 pre-existing `benchmarks` import failure unrelated to this change).
