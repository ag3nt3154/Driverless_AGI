...
- **`system_prompt_preamble` injection differs structurally between `__init__` and rebuild methods**: `__init__` uses a `preamble_parts: list[str]` accumulator that is joined with `\n\n---\n\n` before the base prompt — preamble is one element in that list. The two rebuild methods (`_rebuild_for_normal_mode`, `_rebuild_for_plan_mode`) bypass the accumulator entirely and build `new_system` directly from `_prompt_text.format_map(...)`. Preamble is therefore injected by string-prefixing `new_system` after `format_map`. **Critically, only `__init__` injects soul.md and agents.md; the rebuild methods skip them.** If a future refactor introduces a 4th build site, the same prefix pattern must be applied — there is no central assembly point. This structural divergence caused the soul/agents.md bug discovered 2026-06-17.
...
- **TUI never calls `loop.finish()`**: `_agent_work()` in `tui/app.py` (lines 147–161) never calls `loop.finish()` after `loop.run()` completes. The CLI calls it at 6 separate sites. Without it, every TUI session produces an incomplete JSONL log (missing the `session_end` summary record), and subagent cost/token stats are never rolled up. Fix: add `loop.finish()` (or `loop_ref[0].finish()`) in the `_agent_work` finally block.
- **Soul/agents.md dropped after plan-mode transitions**: `_rebuild_for_normal_mode` (line 789) and `_rebuild_for_plan_mode` (line 854) in `agent/loop.py` only re-inject `system_prompt_preamble` — they skip `soul.md` and both `agents.md` files. After entering and exiting plan mode, the agent's system prompt loses its persona and behavioral guidelines for the rest of the session. Only `__init__` (lines 276–293) injects the full preamble stack. Fix: extract preamble assembly into a shared `_build_preamble()` method called from all 3 build sites.
