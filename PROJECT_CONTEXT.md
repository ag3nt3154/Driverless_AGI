...
> Last updated: 2026-06-21 | [README](README.md) | [TODO](TODO.md)
> Session 2026-06-21 (4): Fixed B2 — `_DAGI_ROOT` centralized in `agent/__init__.py` as `DAGI_ROOT`. All four independent `Path(__file__).parent.parent` expressions in `agent/tools.py`, `tools/cli_subagent.py`, `tools/_subagent_runner.py`, and `agent/loop.py` (×4 local vars) now import from the single canonical definition.
> Session 2026-06-21 (3): Fixed A3 — `_handle_switch_model` now snapshots and restores `provider_order`. Added `provider_order` to `_base_config_snapshot` (init), restored it on `"default"` switch, and copied it from `tier_cfg` on plan/worker switch. All three sites in `agent/loop.py`.
> Session 2026-06-21 (2): README install section updated — added conda + venv paths using requirements.txt, troubleshooting section for OpenAI credential errors (missing .env / langchain), proxy/auth issues (no_proxy env var), and TUI-first workflow (/wd to navigate to project, then /init). No code changes.
> Session 2026-06-21: Fixed A1 — compaction failure no longer crashes the session. `_compact_context()` in `agent/loop.py` now wraps `compact_tool.compact()` in try/except; on failure emits a warning via `on_assistant_text` and returns `_NO_COMPACTION`. Added `_NO_COMPACTION` to the import. Session continues with un-compacted messages.
> Session 2026-06-20: Scheduled review — discovered compaction failure crashes sessions (unhandled re-raise after rollback), base64 image data dumped into compaction summarization prompt (~30K tokens per image), `thinking_tokens` not recorded in JSONL session logs. Also found `_parse_frontmatter` duplicated between skills.py and workflows.py, `_DAGI_ROOT` computed independently in 4+ modules. 17 findings logged in `_todo/todo_2026-06-20.md`.
...
> Session 2026-06-18 (2): Scheduled review — discovered `_rebuild_for_normal_mode` missing `memory_root` kwarg (same divergence class as the `_build_preamble` bug), sidebar reading stale `soul.md` path in `tui/utils.py:66`, BashTool missing `TimeoutExpired` handling, session cost tracking almost always blank. 13 findings logged in `_todo/todo_2026-06-18.md`.
...
- **Preamble assembly is now centralised in `_build_preamble(dagi_root)`** (fixed 2026-06-17): Previously, `__init__` assembled the full preamble stack (system_prompt_preamble + soul.md + dagi agents.md + project agents.md) inline, while `_rebuild_for_normal_mode` and `_rebuild_for_plan_mode` only re-injected `system_prompt_preamble`. The fix extracted the assembly into `AgentLoop._build_preamble(dagi_root: Path) -> str`, called identically from all 3 build sites. If a 4th build site is ever added, it must call `_build_preamble` — there is now a single canonical assembly point.
...
- **`_DAGI_ROOT` centralized in `agent/__init__.py`** — **FIXED 2026-06-21**: Previously, `agent/tools.py`, `tools/cli_subagent.py`, `tools/_subagent_runner.py`, and `agent/loop.py` each independently computed `Path(__file__).parent.parent`. Moving any module to a different directory depth would silently break. Fixed by defining `DAGI_ROOT = Path(__file__).parent.parent` once in `agent/__init__.py`; all four sites now import it (`from agent import DAGI_ROOT`). The four local vars in `loop.py` (lines 215, 608, 653, 892) are replaced with `dagi_root = DAGI_ROOT`.
- **Tool result content not truncated in JSONL logs**: `record_tool_end(name, result_str)` writes the full result string. For large file reads or base64-encoded images, this can be enormous. Compare with `record_subagent_end` which truncates to 500 chars. The 196 accumulated session logs are the primary driver of disk consumption.
...
- **`json.loads(tc.function.arguments)` is parsed up to 4 times per tool call**: In the tool dispatch loop (lines 547–562 of `agent/loop.py`), the same arguments string is parsed at line 547 (dispatch), and again at lines 550, 552, 562 for sentinel handlers. If the JSON is malformed, the dispatch on line 547 catches it via registry error handling, but the sentinel checks on subsequent lines re-parse the same bad string and raise an unhandled `json.JSONDecodeError`.
- **`_effective_memory_root` is recomputed inline in both rebuild methods**: Lines 773–776 and 838–841 of `agent/loop.py` recompute the memory root instead of using `self._effective_memory_root` (already set in `__init__` at line 228). The logic is identical — pure duplication.
...
- **`_rebuild_for_normal_mode` missing `memory_root=` in `create_tool_registry` call** — discovered 2026-06-18. Same class of divergence bug as the `_build_preamble` issue: `__init__` (line 260) and `_rebuild_for_plan_mode` (line 846) both pass `memory_root=self._effective_memory_root`, but `_rebuild_for_normal_mode` (line 779) omits it. After a plan→normal transition with a custom `memory_root` configured, the `SkillTool` silently reverts to the default `project_path / "dagi-memory"`. See `_todo/todo_2026-06-18.md` A1.
