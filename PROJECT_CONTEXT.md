...
> Last updated: 2026-07-14 | [README](README.md) | [TODO](TODO.md)
> Session 2026-07-14 (review): 1 new finding added to TODO.md (`agent/loop.py` at 1013 lines — 2× over 500-line standard, replaces stale "Split cli.py" item since cli.py was archived); 1 item downgraded (image crash — appears already guarded in current code at line 563, needs manual verification); 1 item downgraded to LOW (cli.py plan_mode_exited — dead code in archives/cli.py); CRLF write_text item updated with 3 new sites (subagent_main.py, escalate_issue.py, cli_utils.py); dead code item updated (plan_subagent.py deleted, _resolve_option archived). GNHF dormant 79 days (243 session logs). 8 temp files in .dagi/temp/.
> Session 2026-07-14 (feature, DAGI Eval Benchmark, all 11 tasks complete): Built `benchmarks/dagi_eval/` — a coding-speedup + data-science scorecard harness, deliberately with **no composite score**. New package: subprocess entry executor, `scoring.py` (output-match/timing/ROC-AUC), `harness.py` (workspace prep, canned-solver + real-agent invocation), `run.py` CLI (`--model`, `--task`, `--solver agent|gold|naive`, `--label`, `--results`; `--solver` defaults to `"agent"` — **never invoke without an explicit `--solver naive`/`--solver gold` flag unless the user has explicitly authorized a real LLM call**). 6 tasks: 5 coding-speedup (`coding_01_logpipe`..`coding_05_sheetcalc`, each with a naive baseline + fast gold solution + a `gold_min_speedup` gate: 10.0/10.0/8.0/15.0/25.0, actuals 223.3/49.81/132.33/106.07/1628.45) and 1 DS task (`ds_01_tabular`, ROC-AUC vs. a frozen synthetic dataset with a deliberate leaky-feature trap column `audit_flag`, oracle_auc=0.9191/baseline_auc=0.6635). Two justified deviations from the plan's literal code in `ds_01_tabular` (spec-reviewer approved): (1) `X[c].dtype == object` → `pd.api.types.is_numeric_dtype(X[c])` — pandas 3.0.3 in this env uses native `str` dtype for CSV columns, not `object`; the literal check silently misclassified columns and crashed `SimpleImputer`. (2) `NOISE_SCALE` `1.1`→`2.0` plus a new `MAIN_COEFS` linear main-effect term added to the generator's logit — the plan's literal knobs gave oracle=0.82/baseline=0.53 in this environment (below target ranges) because every DGP signal term was segment-gated and invisible to a non-interacting logistic-regression baseline; flagged as sitting at the edge of "calibration" vs. "changing the DGP" but approved. Full harness self-test (`--solver naive` then `--solver gold`, all 6 tasks): naive speedups 0.81-1.01/`ds_score=1.0`, gold speedups 49.8-1628.5 (all clearing gates)/`ds_score=1.537`, `errors: []` both runs. Full suite 301 passed (was 280; the extra 21 came from `main`'s independent concurrent TUI work absorbed at merge). Task 11's optional real-API smoke test (needs a billed LLM call) was intentionally skipped per standing instruction not to make real LLM calls without explicit permission — documented as an open follow-up in TODO.md. Implemented via `superpowers:subagent-driven-development` in an isolated worktree (`.worktrees/dagi-eval-benchmark`, branch `feature/dagi-eval-benchmark`); merged locally into `main` as `8fcd997` (2 conflicts in `TODO.md`/`requirements.txt`, both "independent additions near the same location" — resolved by keeping both blocks). Worktree removed, branch deleted. Spec: `docs/superpowers/specs/2026-07-06-dagi-eval-benchmark-design.md`; plan: `docs/superpowers/plans/2026-07-06-dagi-eval-benchmark.md`.
> Session 2026-07-13 (feature, TUI Windows notifications): Added native Windows toast notifications to `tui.py` only, firing at three points: DAGI asks a question, DAGI presents a plan for interactive review, DAGI reaches end-of-response. New `tui/notifications.py::notify(title, message)` lazily imports `win11toast` inside a `try/except Exception` — degrades silently to a no-op on non-Windows hosts, missing dependency, or any WinRT toast failure; message truncated to 200 chars. New `AgentCallbacks.on_plan_shown: Callable[[], None]` field (`agent/loop.py`) added specifically to distinguish "plan shown for review" from a generic `on_ask_user` question, rather than string-matching the "Do you have any modifications?" prompt text — fired by `tools/show_plan.py::ShowPlanTool.run()` only in interactive mode (autonomous mode has no human waiting). `tui/callbacks.py::build_callbacks()` wires all three: `on_ask_user` fires `notify("DAGI has a question", question)`, new `on_plan_shown` closure fires `notify("DAGI's plan is ready", ...)`, and `on_done` (previously a no-op lambda) now fires `notify("DAGI is done", result or "Response complete.")`. `notify()` is called directly (unwrapped by `app.call_from_thread()`) since it's an OS-level call, not a Textual widget mutation, and `AgentLoop.run()` already executes on a plain background thread. `cli.py` and `tg/callbacks.py` (Telegram) are explicitly untouched — out of scope. `win11toast>=0.35` added to `requirements.txt` as an active (uncommented) dependency. Implemented via `superpowers:subagent-driven-development` — one implementer + 2-stage review (spec compliance, code quality) per task, 5 code tasks, all approved. Full suite: 272 passed (was 260). Design: `docs/superpowers/specs/2026-07-13-windows-notifications-design.md`; plan: `docs/superpowers/plans/2026-07-13-windows-notifications.md`. Commits: `9bafe40`, `b274cfb`, `d434e86`, `b8ade49`, `90da566`.
...
> Earlier sessions compressed 2026-07-14: 17 sessions from 2026-07-05 to 2026-07-14 + prior compressions back to 2026-06-17 — plan-work-review resilience (escalation channel + live plan status board), /wd sidebar refresh, git workflow (8 tools, dagi/* guard, per-subtask commits), PromptInput test quality, DAGI eval benchmark tasks, compose mode hardening, multiline input bindings, Windows CRLF triple-fix, 4 review sessions (Fable code review, wiki accumulation, CRLF finding). See git log for full history.
...
Driverless AGI (dagi) is a self-hosted, OpenAI-compatible agentic coding assistant built entirely in Python. It takes a task from the user, runs a Plan→Act→Observe loop calling tools (read, write, edit, bash, grep, web search, etc.) until the task is complete, and surfaces results via a Textual TUI (`tui.py`), Telegram bot (`telegram_bot.py`), or single-shot `main.py` entry point. See [README](README.md) for setup and usage.
...
Non-goals: cloud hosting, multi-user auth, UI beyond terminal/Telegram.
...
- **Run (TUI):** `conda run -n dagi python tui.py` (recommended)
- **Run (Telegram):** `conda run -n dagi python telegram_bot.py`
...
│   └── dagi_eval/      # Coding-speedup + DS scorecard harness (run.py, harness.py, scoring.py, tasks/)
...
├── tui.py              # TUI entry point (thin launcher) — recommended
├── telegram_bot.py     # Telegram bot entry point (thin launcher)
├── archives/cli.py     # Rich REPL entry point (DEPRECATED — dead code since 2026-07-12)
...
**Subagents:** Pipe-based subprocess spawning (`tools/_subagent_runner.py`). Each subagent type has a `.dagi/subagents/<type>/subagent_config.yaml` (explicit `tools:` list + `model_tier`). Handoff path generated by `SpawnSubagentTool`, written by subagent on exit. Subagent stdout (newline-delimited JSON events) relayed to main TUI `ConversationPane` with `[subagent-type]` label prefix. `run_subagent()` spawns `tools/subagent_main.py` via `python -m tools.subagent_main` (module invocation, not a file path — running it by path would put `tools/` at `sys.path[0]` and let `tools/copy.py` shadow the stdlib `copy` module). Subprocess gets `cwd=_DAGI_ROOT` and `PYTHONPATH=_DAGI_ROOT` so root-level packages are always resolvable.  
...
| `tui/callbacks.py` | `build_callbacks(app, loop_ref) → AgentCallbacks` free function; `TYPE_CHECKING` guard for circular import; wires `on_ask_user`/`on_plan_shown`/`on_done` to `tui/notifications.py::notify()` |
| `tui/notifications.py` | `notify(title, message)` — best-effort native Windows toast via lazily-imported `win11toast`; `try/except Exception: pass` degrades to silent no-op on non-Windows hosts, missing dependency, or WinRT failure; message truncated to 200 chars |
| `archives/cli.py` | **Archived** Rich REPL — dead code since 2026-07-12, not imported or executed by any live path |
...
| `benchmarks/dagi_eval/run.py` | CLI entry (`python -m benchmarks.dagi_eval.run`); `main()` parses `--model/--label/--task/--timeout-min/--solver/--tasks-dir/--results`; `run_task()` dispatches on `task.yaml`'s `kind` (`"coding"` → `scoring.score_coding_task`, else → `scoring.score_ds_task`), catches all per-task exceptions so one bad task never aborts the sweep; appends one scorecard row per run to `results.jsonl`. `--solver` choices `["agent","gold","naive"]`, **defaults to `"agent"`** — real billed LLM call unless overridden |
| `benchmarks/dagi_eval/harness.py` | Workspace prep (copies `public/` + frozen inputs to a temp dir) and solver dispatch — canned `gold`/`naive` solutions read straight from `hidden/`, `agent` solver drives a real `AgentLoop` against the task workspace |
| `benchmarks/dagi_eval/scoring.py` | `score_coding_task` (subprocess-executes the entry function, times it, diffs output against baseline, computes speedup, checks `gold_min_speedup` gate) and `score_ds_task` (ROC-AUC of submitted predictions vs. `hidden/test_labels.csv`) |
| `benchmarks/dagi_eval/tasks/coding_0{1-5}_*/` | 5 coding-speedup tasks, each `public/` (spec+baseline) + `hidden/gold_solution/`; `gold_min_speedup` gates 10.0/10.0/8.0/15.0/25.0 |
| `benchmarks/dagi_eval/tasks/ds_01_tabular/` | DS scorecard task: frozen synthetic dataset (`public/train.csv`, `public/test_features.csv`, `hidden/test_labels.csv`, `hidden/meta.json`), `generator.py` to regenerate, `hidden/baseline.py` (LogisticRegression) + `hidden/gold_solution/solve.py` (HistGradientBoostingClassifier, drops leaky `audit_flag` trap column) |
...
- **2026-07-14 Bug (dagi_eval Task 10, `ds_01_tabular`)**: The plan's literal `baseline.py`/`solve.py` code used `X[c].dtype == object` to split numeric vs. categorical columns; this silently misclassified string columns and crashed `SimpleImputer`.
  **Cause**: pandas 3.0.3 (installed in the `dagi` conda env) uses its newer native `str` dtype for CSV-loaded string columns instead of the legacy `object` dtype the plan's code assumed.
  **Fix**: Replaced with `pd.api.types.is_numeric_dtype(X[c])` in both `hidden/baseline.py` and `hidden/gold_solution/solve.py` — dtype-agnostic and portable across pandas versions. Spec reviewer confirmed this as a necessary correctness fix, not scope creep.

- **2026-07-14 Calibration note (dagi_eval Task 10, `ds_01_tabular` generator)**: The plan's literal `generator.py` constants (`NOISE_SCALE=1.1`, no main-effect term) produced oracle_auc=0.82/baseline_auc=0.53 in this environment — both below the plan's target ranges (0.88-0.92 oracle / 0.65-0.72 baseline) — because every DGP signal term was segment-gated (interaction-only) and therefore invisible to a non-interacting `LogisticRegression` baseline, starving it of any signal at all.
  **Fix**: Raised `NOISE_SCALE` to `2.0` and added a `MAIN_COEFS=(1.5, 1.5)` linear main-effect term (on `x1` and a `tier==1` indicator) to the logit, giving the baseline a non-interacting signal to partially pick up on while the gold solver still needs the segment-gated interactions to reach the target oracle AUC. Judged by the spec reviewer as legitimate calibration engineering within the plan's design intent, but explicitly flagged as sitting at the edge of "calibration" vs. "changing the DGP" — worth revisiting if the DS task roster grows and a stricter DGP-fidelity bar is wanted.

- **2026-07-14 Environment note (dagi_eval Task 11, self-test execution)**: Two shell friction points recurred throughout this session's worktree work: (1) `cd` with a Windows-backslash path (e.g. `cd C:\Users\...\dagi-eval-benchmark`) silently fails in git-bash — the backslashes get swallowed as escape characters; must use the forward-slash form (`cd "/c/Users/alexr/Driverless_AGI/.worktrees/..."`). (2) bare `conda` is not on PATH in this git-bash shell (`conda: command not found`, exit 127) — every invocation needs the full path `"C:\Users\alexr\miniconda3\Scripts\conda.exe" run -n dagi ...`. Neither is a code bug; both are shell-environment quirks worth remembering for future sessions running long conda/pytest commands from this worktree setup.
...
- **Windows toast notifications are TUI-only, wired via dedicated single-purpose callbacks**: `tui/notifications.py::notify(title, message)` is called directly (unwrapped by `App.call_from_thread()`) from `tui/callbacks.py::build_callbacks()`'s `on_ask_user`, new `on_plan_shown`, and `on_done` closures. Safe to call unwrapped because `win11toast.notify()`'s synchronous path is plain WinRT/COM (verified by reading the installed package source during code review) — no asyncio event-loop dependency — and `AgentLoop.run()` already executes on a plain background `threading.Thread`, not Textual's main loop. `on_plan_shown` was added as a new dedicated `AgentCallbacks` field rather than string-matching `ShowPlanTool`'s `"Do you have any modifications?"` question text, consistent with how other lifecycle events (`on_compaction`, `on_model_switch`, `on_continue_injected`) are each modeled as their own callback rather than inferred from message content.
...
- **Review velocity outpacing fix velocity** (observed 2026-06-26, updated 2026-07-14): 20 reviews accumulated ~35 persisting items, 1 commit in **19 days** (e39e146 fixed 3 items). ~1–2 new items/review vs. 0.2 fixes applied. A focused sprint on the 12 XS-effort items (~4 hours) would halve the persist table. GNHF loop dormant **79 days** despite 243 unanalysed session logs — largest gap between stated goal ("self-improving agent") and current trajectory. **2026-07-03**: reworked `review-session` (free-text selection + single cross-session report, see Notable Points) directly targets this — closes the tooling gap that made per-session review low-signal, but the loop is still dormant until someone actually invokes it against the backlog.
...
- Comfortable delegating an entire multi-task feature (11-task DAGI Eval Benchmark plan) to fully autonomous subagent-driven execution in an isolated worktree, without check-ins between tasks — but draws a hard, explicitly-repeated line around real LLM API spend: "don't run dagi with LLM calls unless I explicitly allow it" was stated as a standing rule to be honored even inside a self-test/smoke-test step that the plan itself called for.
- When given the finishing-a-development-branch menu, chose "merge locally" over "push + PR" for a solo-authored, fully-reviewed feature branch — consistent with the "works directly on `main`" tendency noted above; a PR/review step outside DAGI's own subagent review loop wasn't needed here.

### New this session (2026-07-14, DAGI Eval Benchmark)

- **Subagents self-report cyclomatic complexity unreliably**: across two different tasks (Task 8 and Task 9), an implementer subagent's self-reported CC for the same function came in *lower* than an independent `radon` recomputation by the code-quality reviewer (e.g. self-reported 7 vs. actual 8 in `Engine._ev`/`eval_ast`). Not a single fluke — worth treating self-reported complexity numbers from implementer subagents as optimistic estimates that must always be independently reverified with a real tool, not trusted at face value in review.
- **Stalled subagents cannot be resumed in this environment**: confirmed twice now (this session and previously) that there is no `SendMessage`-equivalent tool available to resume a subagent that stalls mid-task waiting on a background process it started (e.g. "I'll stop polling now and wait for the monitor's completion notification..."). The only working recovery path is for the orchestrating agent to take over directly — verify `git status`, independently re-run whatever the stalled agent hadn't finished, and produce the final verdict itself. This is a structural limitation of the current harness, not a one-off glitch, and should be assumed for any future long-running final-review subagent dispatch.
- **Large raw `git diff main..HEAD --stat` counts can be pure branch-drift noise, not a defect signal**: a long-lived feature branch showed 99 files / 53820 insertions against `main` purely because `main` had 15 independent commits land during the feature branch's lifetime; the actual scoped diff (path-restricted to the feature's own directories) was a clean 64 files / 53655 insertions. Before treating a huge diff stat as a review red flag, restrict the diff to the feature's actual paths first.
...
- **`win11toast`**: Windows-only toast notification library; `tui/notifications.py` degrades to a silent no-op if missing or on non-Windows hosts, so this dependency has no availability risk to the agent loop itself — only to the notification UX feature.
...
- **DAGI Eval Benchmark has no real-agent run yet**: `benchmarks/dagi_eval/` is fully self-tested with `--solver naive`/`--solver gold` (canned solutions, zero LLM calls) but has never been run with `--solver agent` against a real model — needs explicit user permission (standing instruction) plus a budgeted API key before the first real scorecard row can be produced in `results.jsonl`.
- **GNHF self-review dormant 79 days**: 243 unanalysed session logs (last: 2026-07-13). `review-session` was reworked 2026-07-03 (free-text selection + single cross-session report) specifically to make a batch run high-signal instead of N repetitive per-session write-ups — the tooling gap is closed, but no bootstrap run has been invoked yet. See TODO.md Work Queue / Self-Improvement Queue for the pending "review the 10 most recent sessions" action.
