...
- **`_DAGI_ROOT` computed independently in 4+ modules**: `agent/tools.py:41`, `tools/cli_subagent.py:18`, `tools/_subagent_runner.py:19`, and `agent/loop.py:215` (local var) all use `Path(__file__).parent.parent`. Moving any module to a different depth breaks silently. A central `DAGI_ROOT` in `agent/__init__.py` would be more robust.
