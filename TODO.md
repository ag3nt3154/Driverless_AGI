...
- **Provider call has no timeout — worker can block for ~30 min** · `open` · found `2026-08-26` — `AgentLoop` builds `openai.OpenAI(api_key=…, base_url=…)` with no `timeout`, so a stalled provider response falls back to the SDK default (600s read, `max_retries=2`) before `run()` sees an error. Evidence: three GUI sessions (`session_2026-08-24_10-26-17`, `…_10-28-09`, `session_2026-08-24_05-46-57`) end at `plan/write` with no `turn/end` and no `finally: idle` in `pyside_worker.log` — worker thread alive, UI thread alive, app force-killed. Same day shows real provider stalls (`APIError: Upstream idle timeout exceeded` after 159s, `Provider returned error`, `model returned a null response`). Fix: config-backed `request_timeout` passed to the client (and to `_generate_session_slug`, which is also untimed), so a stall surfaces as a retryable `APITimeoutError` in seconds rather than a silent multi-minute freeze.

- **Stale lifecycle tests after the drift refactor** · `done` · `2026-08-26` — Retargeted `test_pause_race_cannot_drift_after_paused`, `test_affect_listener_can_reenter_inject_and_resume`, and `test_pause_during_affect_resolution_prevents_late_drift_after_pause_returns` to call `_lifecycle.publish_affect_drift()` directly (matching the timer-based drift path). Restored `_start_drift_timer`/`_stop_drift_timer` in `agent/loop.py` which were accidentally dropped during the loop-split refactor. All 48 lifecycle tests pass.

- **`pyside_gui/app.py` file cap is stale** · `open` · found `2026-08-26` — `test_pyside_app_stays_under_file_cap` asserts ≤500 lines; the file has been 547+ on `main` since the left-sidebar work. Raise the cap or split the module.

## Completed

- **Plan mode removed — replaced with `/plan` skill + `create_plan` tool** · `done` · `2026-09-05` — Removed plan mode as a system-level infrastructure feature (~849 lines deleted, ~211 lines added across 32 files). (1) `SideEffect.ENTER_PLAN_MODE`/`EXIT_PLAN_MODE` removed from `agent/protocol.py`. (2) `AgentConfig` fields `plan_mode`, `plan_file`, `plan_mode_initiated_by`, `previous_branch` removed; `plan_mode_initiated_by` replaced by `autonomous: bool`. (3) `agent/tools.py` collapsed plan-mode if/else branching into a single flat path — all tools always registered, no more tool restriction or registry rebuild. (4) New `tools/create_plan/` tool for plan file scaffolding. (5) `tools/plan_mode/` deleted entirely (`EnterPlanModeTool`, `ExitPlanModeTool`). (6) `agent/_plan_mode.py` gutted to backward-compat stub; `agent/_reload.py` extracted with `rebuild_for_reload`. (7) `.dagi/skills/plan/SKILL.md` rewritten to use `create_plan`, bash for git branching, `switch_model` for model switching, and `set_active_plan` for persistence. (8) `scheduler/runner.py` uses `config.autonomous = True` instead of `plan_mode_initiated_by`. (9) Tests `test_plan_mode_branch.py` and `test_plan_mode_extract.py` deleted; remaining test suites updated.

- **Deliver workflow (Tasks 1–8)** · `done` · `2026-09-05` — Full Plan-Work-Review delivery workflow implemented. (1) `tools/_subagent_runner.py`: output tee to ring buffer + log file, thread-join ordering, `output_tail`/`output_log_path` in all result dicts. (2) `tools/subagent_api.py`: `SubagentResult` extended with `message`, `exit_code`, `output_tail`, `output_log_path`. (3) `tools/_handoff_format.py`: shared `format_error_result` formatter, `MISSING_HANDOFF_NOTICE`. (4) `.dagi/subagents/review/main.py`: general-purpose `review_work(material, passing_criteria, context, verification)` — no active plan required. (5) `.dagi/subagents/review/prompt.md` + `worker/prompt.md`: PASS/ESCALATE and READY_FOR_REVIEW/ESCALATE outcomes; workers now see and run tests. (6) `tools/active_plan/`: `SetActivePlanTool` + `CheckActivePlanTool` with sidecar at `.dagi/session-state/<thread_id>/active-plan.json`; `SideEffect.SET_ACTIVE_PLAN` wired into dispatch. (7) `agent/_plan_mode.py`: `handle_all_tasks_resolved` retains association; `handle_exit_plan_mode` writes sidecar on success, skips on cancel. (8) `.dagi/skills/deliver/SKILL.md`: canonical orchestration skill with plan template. (9) Legacy cleanup: `dagi-execute` converted to shim, `agent/dynamic_context.py` deleted, `review_utils.py` deleted. Tests: 1032 passing. Plan: `docs/superpowers/plans/2026-09-05-deliver-workflow.md`.

- **String-Sentinel Protocol Refactor** · `done` · `2026-08-26` — Replaced all in-band string-sentinel control flow (`<<END_OF_RESPONSE>>`, `<<HANDOFF_WRITTEN>>`, etc.) with a typed `ToolResult` return protocol. (1) `agent/protocol.py`: `ToolResult(output, side_effect, side_effect_data)` dataclass + `SideEffect` enum (`END_TURN`, `ENTER_PLAN_MODE`, `EXIT_PLAN_MODE`, `ALL_TASKS_RESOLVED`, `RELOAD_SKILLS`, `SWITCH_MODEL`). (2) `agent/base_tool.py` + `agent/registry.py`: widened `run()`/`dispatch()` return type to `str | ToolResult`. (3) All sentinel tools converted: `WriteHandoffTool` (dual-mode: main agent returns `ToolResult(side_effect=END_TURN)` without writing file; subagent writes file + returns `ToolResult`), `UpdateTaskStatusTool` (returns `ToolResult(side_effect=ALL_TASKS_RESOLVED)` when done), `EmoteTool`, `EnterPlanModeTool`, `ExitPlanModeTool`, etc. (4) `agent/loop.py` + `agent/_tool_dispatch.py`: dispatch reads `ToolResult.side_effect` instead of scanning text for sentinel strings. (5) `.dagi/prompts/main/continue.md` updated to instruct model to call `write_handoff` instead of emitting `<<END_OF_RESPONSE>>`. (6) `SESSION_CONTEXT_HEADER` consolidated into `agent/protocol.py`; `agent/dynamic_context.py` and `agent/session_log.py` import it. (7) `tools/subagent_main.py`: removed `<<END_OF_RESPONSE>>` from `_validate_final_handoff`. Updated test suites: `test_agent_loop.py` (48 pass), `test_session_log_shadow.py` (46 pass), `test_tool_filter.py` (13 pass), `test_update_task_status.py` (7 pass), `test_output_filter.py` (19 pass). Full suite: 1040+ passing.

- **TUI + GUI freeze — stale `ask_user` sink swallows the next message** · `done` · `2026-08-26` — Symptom: running timer keeps counting, process channel reads `idle`, prompt disabled, ESC does nothing. Root cause: `_pending_ask` is UI state that outlives the worker that raised the question. A timed-out `ask_user` leaves the sink in place; the next message is posted into a dead `threading.Event` and the running indicator is shown with no worker started. The `2026-08-23` GUI fix only covered `_on_agent_done`/`_on_agent_paused`, so turns ending via `on_error` or an exception still stranded it — and the TUI was never fixed at all (it also blocks ESC recovery via `action_pause`'s `_pending_ask` guard). Fix, both UIs: (1) submit only routes to the answer branch while the asking thread is alive (`DagiApp._ask_is_live`, worker check in `DagiMainWindow._on_input_submitted`), otherwise the stale sink is dropped and the text starts a normal turn; (2) `_agent_work`'s `finally` retires the question on every exit path (`_clear_pending_ask` / `_clear_pending_ask_slot`). Tests: `tests/test_tui_pending_ask.py`, `pyside_gui/tests/test_pending_ask.py` (10 passing, red before the fix).

- **VAD drift timer** · `done` · `2026-08-24` — Affect drift is now timer-based instead of step-based, decoupled from agent step boundaries. A periodic `threading.Timer` fires drift every `drift_interval` seconds (default 10s, configurable via `affect.drift_interval` in config YAML) while the agent loop is running, matching the sidebar GIF rotation pace. New `drift_interval` field on `AffectConfig`, threaded through `AgentConfig.affect_drift_interval`. `_start_drift_timer`/`_stop_drift_timer` manage the timer lifecycle in `AgentLoop.run()`; `_continuing_step_finished` no longer triggers drift.

- **PySide6 left sidebar dynamic resize + file viewer word-wrap** · `done` · `2026-08-24` — Sidebar no longer uses fixed widths; `LeftSidebar` emits `expansion_changed(bool)` and `DagiMainWindow._on_sidebar_expansion()` redistributes splitter sizes (half the conversation pane on expand, collapses back to 40px rail). Removed `_PANEL_WIDTH` constant; panel uses `stretch=1` and `setMinimumWidth(_RAIL_WIDTH)` so the splitter controls width. `FileViewerView` line wrap changed from `NoWrap` to `WidgetWidth`; CSS gains `word-wrap: break-word` on body/pre/td and proper `ul`/`ol` list styles. Suppresses spurious mid-task notifications; clears stale `_pending_ask` on agent done.

- **PySide GUI freeze after ask_user timeout** · `done` · `2026-08-23` — Root cause: stale `_pending_ask` reference left dangling after a timed-out `ask_user` call would swallow the next user message, showing a running timer but starting no worker thread. Fix: clear `_pending_ask` in `_on_agent_done` and `_on_agent_paused`. Also added file-based worker stage logging (`pyside_worker.log`) independent of QWebEngineView to capture future stall diagnostics. 47 PySide tests passing.

- **PySide6 left sidebar — collapsible rail + three modal views** · `done` · `2026-08-23` — Replaced the session-history-only `LeftSidebar` with a collapsible icon-rail sidebar (40px rail always visible; 280px panel expands on click). Three modal views in a `QStackedWidget`: (1) `SessionHistoryView` — two-line list, lazy session loading; (2) `FileTreeView` — `QFileSystemModel` + `_FilterProxy` hiding `.git`, `__pycache__`, `.dagi/logs`, `node_modules`, `*.pyc`; (3) `FileViewerView` — `_TextEditor`+`LineNumberArea` for plain text, `QWebEngineView` for `.md`, 500KB cap, UTF-8+replace. `LeftSidebar` coordinator: `activate_view()`, `collapse()`, `is_expanded()`, `set_project_path()`, `open_file()`; bubbles `session_selected`. `/hist` → `activate_view("history")`; `/wd` propagates to both sidebars. 46 tests passing. Spec: `docs/superpowers/specs/2026-08-23-left-sidebar-design.md`.

- **Main/inherited `write_handoff` final-action restoration** · `done` · `2026-08-20` — Added `write_handoff` as an unfilterable main-agent lifecycle tool backed by safe `.dagi/handoffs/main_<thread-hash12>.md` paths; main calls render the full Markdown in non-verbose and verbose TUI modes and terminate immediately without `END_OF_RESPONSE`. The reserved lifecycle name replaces any colliding project tool without aborting startup. Version-2 inherited children reuse the exact parent-visible schema, receive a path-bound implementation plus allowlist exemption, must call the tool as their final action, and validate the written file with one corrective retry before hard failure. Removed inherited final-assistant-text persistence. Affected regression suite: 202 passing before final review; review regressions cover display, collision handling, and path containment.
...
- **Split `agent/loop.py` into focused modules** · `done` · `2026-08-26` — executed a task-by-task TDD extraction on branch `dagi-split-agent-loop` (1,821 → 789 lines; every extracted module ≤240). New internal modules (all re-exported via `agent.loop` for backward compatibility): `_loop_config.py` (AgentConfig/AgentCallbacks/CompactionResult), `_loop_helpers.py` (sentinels, CONTINUE_PROMPT, wiki-index + reload notification helpers), `_system_prompt.py` (`assemble_system_string` — single source of truth), `_plan_mode.py` (enter/exit/rebuild/reload handlers), `_model_switch.py` (tier switching + deduplicated `build_extra_body`), `_streaming.py` (`consume_stream`), `_compaction.py` (forked compact-subagent pipeline), `_tool_dispatch.py` (dispatch/bookkeeping/write_handoff short-circuit). Each move was verbatim (`self` → explicit `loop` param), gated by the full suite after every commit; baseline 5 pre-existing failures unchanged throughout. White-box test patch targets updated where call sites moved (`agent.loop.run_subagent` → `agent._compaction.run_subagent`, `create_task_branch` → `agent._plan_mode`). Residual: `loop.py` still 789 lines because `run()` (~280) and `__init__` (~150) were deliberately retained per plan notes — further reduction needs a follow-up plan decomposing `run()`'s retry machinery. Gate runs used `--noconftest`: the RAM-watchdog conftest fires at ambient machine RAM ≥70%. Plan: `.dagi/plans/split-agent-loop/plan.md`.
...
- **Tests for `_rebuild_for_reload` state preservation** · `done` · `2026-09-05` — plan mode removed; `rebuild_for_reload` no longer has plan-mode state to preserve. Autonomous mode now uses the `autonomous` bool on AgentConfig.
