...
- **Stale lifecycle tests after the drift refactor** · `open` · found `2026-08-26` — `tests/test_agent_loop.py::TestProcessLifecycle::{test_pause_race_cannot_drift_after_paused,test_affect_listener_can_reenter_inject_and_resume}` fail on `main`. Both assert that `_continuing_step_finished` publishes affect drift through `LifecyclePublisher`; drift moved to a GUI-level `QTimer` in `9385b7d`, so the loop no longer touches `pause_event` there and the doubles never fire. Either retarget them at `DagiMainWindow._tick_drift` or delete them — as written they no longer guard anything.
