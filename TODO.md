...
- **Main/inherited `write_handoff` final-action restoration** · `done` · `2026-08-20` — Added `write_handoff` as an unfilterable main-agent lifecycle tool backed by `.dagi/handoffs/main_<thread8>.md`; main calls return the full Markdown to the TUI completion path and terminate immediately without `END_OF_RESPONSE`. Version-2 inherited children now reuse the exact parent-visible schema, receive a path-bound implementation plus allowlist exemption, must call the tool as their final action, and validate the written file with one corrective retry before hard failure. Removed inherited final-assistant-text persistence. Affected regression suite: 202 passing.
