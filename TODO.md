...
- **`tg/bot.py` `UnboundLocalError` in `finally` block — masks original exception** · `priority:high` · `open:5d` · `effort:XS`
  - **File:** `tg/bot.py:163`
...
- **Scheduler `loop.finish()` races with daemon thread on timeout** · `priority:medium` · `open:10d` · `effort:XS`
...
- **Telegram `build_callbacks` doesn't wire `on_subagent_event_factory` — subagent output invisible** · `priority:medium` · `open:10d` · `effort:S`
...
- **`tui/commands.py` imports from `cli.py` — layering violation** · `priority:medium` · `open:26d` · `effort:XS`
...
- **Dead code: `PlanSubAgent`, `ExploreFilesTool`, `WebResearchTool`, `SubAgentRunner`** · `priority:high` · `open:26d` · `effort:S`
...
- **Split `cli.py` (1355 lines) → `cli/` package** · `priority:high` · `open:23d` · `effort:M`
...
- **`agent/prompts.py` still uses independent `Path(__file__).parent.parent`** · `priority:medium` · `open:12d` · `effort:XS`
...
- **`_parse_frontmatter` duplicated verbatim between `agent/skills.py` and `agent/workflows.py`** · `priority:medium` · `open:19d` · `effort:XS`
...
- **`_extra_body` construction duplicated in `__init__` and `_handle_switch_model`** · `priority:medium` · `open:20d` · `effort:XS`
...
- **`ask_user` callback has no deadlock protection (infinite wait)** · `priority:medium` · `open:21d` · `effort:XS`
...
- **`_tools_from_list` limited to 9 hardcoded tool names** · `priority:medium` · `open:21d` · `effort:S`
...
- **Sidebar `_system_breakdown` reads stale `soul.md` path** · `priority:medium` · `open:21d` · `effort:XS`
...
- **`_system_breakdown()` reads disk on every Textual render cycle** · `priority:medium` · `open:21d` · `effort:XS`
...
- **`SkillTool.run()` reloads all skills from disk on every invocation** · `priority:medium` · `open:14d` · `effort:S`
...
- **`WebFetchTool` silently upgrades HTTP→HTTPS for private IP addresses** · `priority:medium` · `open:14d` · `effort:XS`
...
- **`filter_tool_output` temp files never cleaned up — unbounded accumulation** · `priority:medium` · `open:7d` · `effort:XS`
...
- **`tg/bot.py:153` uses deprecated `asyncio.get_event_loop()` — Python 3.14 breakage risk** · `priority:medium` · `open:5d` · `effort:XS`
  - **File:** `tg/bot.py:153`
...
- **`BashTool.run()` doesn't handle `subprocess.TimeoutExpired`** · `priority:medium` · `open:21d` · `effort:XS`
...
- **Session cost tracking always shows `$—`** · `priority:high` · `open:21d` · `effort:S`
...
- **`thinking_tokens` (reasoning tokens) not recorded in session JSONL** · `priority:high` · `open:19d` · `effort:S`
...
- **Cache hit visibility in TUI sidebar** · `priority:high` · `open:23d` · `effort:S`
...
- **Tool result content not truncated in JSONL logs** · `priority:medium` · `open:19d` · `effort:XS`
...
- **Wiki index system messages accumulate across `run()` calls — unbounded token waste** · `priority:medium` · `open:0d` · `effort:XS`
...
- **Token efficiency benchmark harness** · `priority:high` · `open:20d` · `effort:M`
...
- **GNHF self-improvement loop — never bootstrapped (74 days stale)** · `priority:high` · `open:74d`
...
- **`langchain` + `langchain-openai` are dead dependencies in `requirements.txt`** · `priority:low` · `open:11d` · `effort:XS`
...
- **Dead `ChatSession.lock` field in `tg/session.py`** · `priority:low` · `open:9d` · `effort:XS`
...
- **`config.example.yaml:85` stale BM25 reference in `memory_root` comment** · `priority:low` · `open:7d` · `effort:XS`
...
- **Telegram bot redundant `config.project_path` assignment** · `priority:low` · `open:9d` · `effort:XS`
  - **File:** `tg/bot.py:137`
...
**Root cause:** The `/improve-yourself` workflow has never been run. Review items are waiting. 209 session logs have accumulated; self-review last ran 74 days ago (2026-04-26).
