...
- **Telegram bot has no authorization — unauthenticated remote code execution** · `priority:critical` · `open:3d` · `effort:S`
...
- **Memory-subagent wiki sandbox fails open when `memory_root` is unset (the default)** · `priority:critical` · `open:3d` · `effort:S`
...
- **Reading any image file crashes the agent loop (`AttributeError` on list result)** · `priority:medium` · `open:3d` · `effort:XS` · `needs-verification`
...
- **`archives/cli.py:1240` `plan_mode_exited` — AttributeError (dead code)** · `priority:low` · `open:3d` · `effort:XS`
...
- **`tg/bot.py` `UnboundLocalError` in `finally` block — masks original exception** · `priority:high` · `open:10d` · `effort:XS`
  - **File:** `tg/bot.py:163`
...
- **`grep` Python fallback silently drops all matches under dotted directories** · `priority:high` · `open:3d` · `effort:XS`
...
- **Scheduler `loop.finish()` races with daemon thread on timeout** · `priority:medium` · `open:15d` · `effort:XS`
...
- **Telegram `build_callbacks` doesn't wire `on_subagent_event_factory` — subagent output invisible** · `priority:medium` · `open:15d` · `effort:S`
...
- **Dead code: `ExploreFilesTool`, `WebResearchTool`, `SubAgentRunner`** · `priority:high` · `open:34d` · `effort:S`
...
- **Split `agent/loop.py` (1013 lines) into focused modules** · `priority:high` · `open:0d` · `effort:M`
...
- **`agent/prompts.py` still uses independent `Path(__file__).parent.parent`** · `priority:medium` · `open:17d` · `effort:XS`
...
- **`_parse_frontmatter` duplicated verbatim between `agent/skills.py` and `agent/workflows.py`** · `priority:medium` · `open:24d` · `effort:XS`
...
- **`_extra_body` construction duplicated in `__init__` and `_handle_switch_model`** · `priority:medium` · `open:25d` · `effort:XS`
...
- **`ask_user` callback has no deadlock protection (infinite wait)** · `priority:medium` · `open:26d` · `effort:XS`
...
- **`_tools_from_list` limited to 9 hardcoded tool names** · `priority:medium` · `open:26d` · `effort:S`
...
- **Sidebar `_system_breakdown` reads stale `soul.md` path** · `priority:medium` · `open:26d` · `effort:XS`
...
- **`_system_breakdown()` reads disk on every Textual render cycle** · `priority:medium` · `open:26d` · `effort:XS`
...
- **`SkillTool.run()` reloads all skills from disk on every invocation** · `priority:medium` · `open:19d` · `effort:S`
...
- **Falsy-zero coercion in config — `reserve_tokens: 0` silently becomes `16384`** · `priority:medium` · `open:3d` · `effort:XS`
...
- **`AskUserTool` double-timeout race — user answers but tool already returned fallback** · `priority:medium` · `open:3d` · `effort:XS`
...
- **`explore_files` schema requires `handoff_file` param the code ignores — token waste** · `priority:medium` · `open:3d` · `effort:XS`
...
- **`WebFetchTool` silently upgrades HTTP→HTTPS for private IP addresses** · `priority:medium` · `open:19d` · `effort:XS`
...
- **`filter_tool_output` temp files never cleaned up — unbounded accumulation** · `priority:medium` · `open:12d` · `effort:XS`
...
- **`tg/bot.py:153` uses deprecated `asyncio.get_event_loop()` — Python 3.14 breakage risk** · `priority:medium` · `open:10d` · `effort:XS`
  - **File:** `tg/bot.py:153`
...
- **Session cost tracking always shows `$—`** · `priority:high` · `open:26d` · `effort:S`
...
- **`thinking_tokens` (reasoning tokens) not recorded in session JSONL** · `priority:high` · `open:24d` · `effort:S`
...
- **Cache hit visibility in TUI sidebar** · `priority:high` · `open:28d` · `effort:S`
...
- **Tool result content not truncated in JSONL logs** · `priority:medium` · `open:24d` · `effort:XS`
...
- **Wiki index system messages accumulate across `run()` calls — unbounded token waste** · `priority:medium` · `open:5d` · `effort:XS`
...
- **Token efficiency benchmark harness** · `priority:high` · `open:25d` · `effort:M`
...
- **`langchain` + `langchain-openai` are dead dependencies in `requirements.txt`** · `priority:low` · `open:16d` · `effort:XS`
...
- **Dead `ChatSession.lock` field in `tg/session.py`** · `priority:low` · `open:14d` · `effort:XS`
...
- **`config.example.yaml:85` stale BM25 reference in `memory_root` comment** · `priority:low` · `open:12d` · `effort:XS`
...
- **Telegram bot redundant `config.project_path` assignment** · `priority:low` · `open:14d` · `effort:XS`
  - **File:** `tg/bot.py:137`
...
**Root cause:** The `/improve-yourself` workflow has never been run. Review items are waiting. 243 session logs have accumulated; self-review last ran 79 days ago (2026-04-26).
