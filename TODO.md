...
- **Streaming delta callbacks wired to `StreamPreview` (Task 6 of streaming plan)** · `done` · `2026-07-17`
  - **Problem:** `AgentLoop._consume_stream` (`agent/loop.py`) already fires `on_stream_start`/`on_stream_end`/`on_assistant_text_delta`/`on_reasoning_delta`, and `tui/streaming.py::StreamPreview` (Task 5) already knows how to render partial text — but `tui/callbacks.py::build_callbacks` left all four fields at their `AgentCallbacks` no-op defaults, so nothing ever reached the widget.
  - **Added:** `build_callbacks` now queries `StreamPreview` alongside the existing `Sidebar`/`ConversationPane` lookups and accumulates deltas in a closure-local dict. `on_stream_start` resets the accumulated reasoning/text (and per-kind throttle timestamps) so a second turn never inherits leftover text from the previous one. `on_assistant_text_delta`/`on_reasoning_delta` append to the accumulator and call `preview.show_progress(reasoning, text)` via `app.call_from_thread`, throttled to ≥50 ms **per delta kind** (`text` vs `reasoning` tracked separately) so a burst of same-kind tokens doesn't flood the UI thread, while every flush still sends the FULL accumulated strings so a skipped refresh loses nothing. `on_stream_end` forces one final flush (bypassing the throttle) before calling `preview.finish()`, guaranteeing the last frame always shows the complete text.
  - **Gotcha found while writing tests:** a single shared `last_flush` timestamp (as sketched in the original plan) breaks the very first reasoning delta whenever it lands immediately after a text delta — two calls made microseconds apart in the same synchronous burst both fall inside a 50 ms window measured from a *single* clock, so the second one gets silently dropped even though it's a different kind of update the user has never seen yet. Splitting the throttle clock per delta kind fixes it: the first arrival of each kind (text or reasoning) always flushes immediately, and only rapid repeats of the *same* kind get throttled.
  - **Test:** `tests/test_tui_callbacks.py::TestStreamingWiring` (4 new tests — reset-and-accumulate, throttled-accumulation-survives-to-forced-flush, stream-end-always-finishes, second-stream-starts-clean). Full suite `pytest tests/ -q` → 415 passed, 7 pre-existing unrelated failures in `tests/dagi_eval/` (`ModuleNotFoundError: numpy` — not installed in the `dagi` conda env).

- **`StreamPreview` TUI widget added (Task 5 of streaming plan)** · `done` · `2026-07-17`
  - **Problem:** `ConversationPane` (`tui/conversation.py`) extends Textual's `RichLog`, which is append-only — once streaming support lands (Task 6), there's nowhere to show growing partial text without leaving stale copies behind in scrollback.
  - **Added:** `tui/streaming.py::StreamPreview` — a `Static` subclass, hidden via `DEFAULT_CSS` (`display: none`) until `show_progress(reasoning, text)` is first called; `finish()` re-hides and clears it once the final text lands in `ConversationPane`. `_render_tail()` renders reasoning (dim italic, prefixed) before the response text, then trims to the last `TAIL_LINES` (12) lines so the preview never crowds out the conversation. Mounted in `tui/app.py::DagiApp.compose()` between `ConversationPane` and `#running-indicator`.
  - **Scope:** purely presentational — wiring to the agent loop's actual stream events landed separately as Task 6 (see above).
  - **Test:** `tests/test_stream_preview.py` (5 tests — hidden by default, `show_progress` makes visible and renders text, `finish` hides and clears, tail-trimming keeps only the newest `TAIL_LINES` lines, reasoning renders before text). Full suite `pytest tests/ -q` → 411 passed (7 pre-existing unrelated failures in `tests/dagi_eval/` due to missing `numpy` in the `dagi` conda env).
...
- **Telegram bot has no authorization — unauthenticated remote code execution** · `priority:critical` · `open:5d` · `effort:S`
...
- **Memory-subagent wiki sandbox fails open when `memory_root` is unset (the default)** · `priority:critical` · `open:5d` · `effort:S`
...
- **Reading any image file crashes the agent loop (`AttributeError` on list result)** · `priority:medium` · `open:5d` · `effort:XS` · `needs-verification`
...
- **`archives/cli.py:1240` `plan_mode_exited` — AttributeError (dead code)** · `priority:low` · `open:5d` · `effort:XS`
...
- **`tg/bot.py` `UnboundLocalError` in `finally` block — masks original exception** · `priority:high` · `open:12d` · `effort:XS`
  - **File:** `tg/bot.py:163`
...
- **`grep` Python fallback silently drops all matches under dotted directories** · `priority:high` · `open:5d` · `effort:XS`
...
- **`BashTool._killed_by_user` race between `run()` and `force_kill()`** · `priority:medium` · `open:0d` · `effort:XS`
...
- **Scheduler `loop.finish()` races with daemon thread on timeout** · `priority:medium` · `open:17d` · `effort:XS`
...
- **Telegram `build_callbacks` doesn't wire `on_subagent_event_factory` — subagent output invisible** · `priority:medium` · `open:17d` · `effort:S`
...
- **Dead code: `ExploreFilesTool`, `WebResearchTool`, `SubAgentRunner`** · `priority:high` · `open:36d` · `effort:S`
...
- **Split `agent/loop.py` (1013 lines) into focused modules** · `priority:high` · `open:2d` · `effort:M`
  - **File:** `agent/loop.py` (1013 lines — 2× over 500-line standard)
  - **Problem:** Mixes core loop execution, plan-mode handling, system-prompt assembly, wiki injection, sentinel parsing, model switching, tool dispatch, compaction, and the live plan-status board rendering. Largest Python file in the live codebase.
...
- **`agent/prompts.py` still uses independent `Path(__file__).parent.parent`** · `priority:medium` · `open:19d` · `effort:XS`
...
- **`_parse_frontmatter` duplicated verbatim between `agent/skills.py` and `agent/workflows.py`** · `priority:medium` · `open:26d` · `effort:XS`
...
- **`_extra_body` construction duplicated in `__init__` and `_handle_switch_model`** · `priority:medium` · `open:27d` · `effort:XS`
...
- **`ask_user` callback has no deadlock protection (infinite wait)** · `priority:medium` · `open:28d` · `effort:XS`
...
- **`_tools_from_list` limited to 9 hardcoded tool names** · `priority:medium` · `open:28d` · `effort:S`
...
- **Sidebar `_system_breakdown` reads stale `soul.md` path** · `priority:medium` · `open:28d` · `effort:XS`
...
- **`_system_breakdown()` reads disk on every Textual render cycle** · `priority:medium` · `open:28d` · `effort:XS`
...
- **`SkillTool.run()` reloads all skills from disk on every invocation** · `priority:medium` · `open:21d` · `effort:S`
...
- **Falsy-zero coercion in config — `reserve_tokens: 0` silently becomes `16384`** · `priority:medium` · `open:5d` · `effort:XS`
...
- **`AskUserTool` double-timeout race — user answers but tool already returned fallback** · `priority:medium` · `open:5d` · `effort:XS`
...
- **`explore_files` schema requires `handoff_file` param the code ignores — token waste** · `priority:medium` · `open:5d` · `effort:XS`
...
- **`WebFetchTool` silently upgrades HTTP→HTTPS for private IP addresses** · `priority:medium` · `open:21d` · `effort:XS`
...
- **Session cost tracking always shows `$—`** · `priority:high` · `open:28d` · `effort:S`
...
- **`thinking_tokens` (reasoning tokens) not recorded in session JSONL** · `priority:high` · `open:26d` · `effort:S`
...
- **Cache hit visibility in TUI sidebar** · `priority:high` · `open:30d` · `effort:S`
...
- **Tool result content not truncated in JSONL logs** · `priority:medium` · `open:26d` · `effort:XS`
...
- **Wiki index system messages accumulate across `run()` calls — unbounded token waste** · `priority:medium` · `open:7d` · `effort:XS`
...
- **Token efficiency benchmark harness** · `priority:high` · `open:27d` · `effort:M`
...
- **`langchain` + `langchain-openai` are dead dependencies in `requirements.txt`** · `priority:low` · `open:18d` · `effort:XS`
...
- **Dead `ChatSession.lock` field in `tg/session.py`** · `priority:low` · `open:16d` · `effort:XS`
...
- **`config.example.yaml:85` stale BM25 reference in `memory_root` comment** · `priority:low` · `open:14d` · `effort:XS`
...
- **Telegram bot redundant `config.project_path` assignment** · `priority:low` · `open:16d` · `effort:XS`
  - **File:** `tg/bot.py:137`
...
**Root cause:** The `/improve-yourself` workflow has never been run. Review items are waiting. 243 session logs have accumulated; self-review last ran 81 days ago (2026-04-26).
